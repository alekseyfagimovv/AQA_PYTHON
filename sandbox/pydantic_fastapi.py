...
class ScooterCreateSchema(BaseModel):
    serial_number: str

@app.post("/scooters/", status_code=status.HTTP_201_CREATED)
def create_scooter(scooter_data: ScooterCreateSchema, db: Session = Depends(get_db)):
    db_scooter = ScooterModel(serial_number=scooter_data.serial_number)
    try:
        db.add(db_scooter)
        db.commit() 
        db.refresh(db_scooter)
        return {"id": db_scooter.id, "serial_number": db_scooter.serial_number}
        
    except IntegrityError:
        db.rollback()
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,   
            detail="Самокат с таким номером уже есть",
        )

# # Изучал теорию 
# Fast api Uvicorn, Starlette и OpenAPI. Понимать что это.
# База SQL ALCHEMY, validates,
# Реляционные связи и ограничения СУБД : FOREIGN KEY, UNIQUE и каскадное удаление, 
# изоляция транзакций  и Конкурентность, продвинутые JOIN. 